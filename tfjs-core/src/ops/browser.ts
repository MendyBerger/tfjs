...
export async function toPixels(
  ...
  // If the current backend has 'toCanvas' registered, it has a more
  ...
  if (kernel != null) {
    ...
    const attrs: ToPixelsOutput = {canvas};
    const outTensor = ENGINE.runKernel(
                          ToPixels, inputs as {} as NamedTensorMap,
